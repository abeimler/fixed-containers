...
public:  // Public so this type is a structural type and can thus be used in template parameters
    rich_enums_detail::RichEnumStorage<BackingEnum> storage;
...
public:
    ...
    constexpr const BackingEnum& backing_enum() const { return storage.value(); }
    /*implicit*/ constexpr operator BackingEnum() const { return storage.value(); }
    ...
    [[nodiscard]] constexpr bool has_value() const { return storage.has_value(); }

    [[nodiscard]] constexpr std::size_t ordinal() const
    {
        return static_cast<std::size_t>(this->storage.value());
    }
...
public:
    ...
    [[nodiscard]] constexpr std::string_view to_string() const
    {
        if (this->has_value())
        {
            return magic_enum::enum_name(this->storage.value());
        }
